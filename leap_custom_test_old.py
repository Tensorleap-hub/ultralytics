...
def check_custom_test():
    ...
    for subset in responses: # [training, validation, test ,unlabeled]
        for idx in range(10):
            ...
            # predict
            y_pred = model.run(None, {model.get_inputs()[0].name: concat})
            ...
            if subset.state != DataStateType.unlabeled:
                ...
                # custom metrics
                total_loss=loss(y_pred[1].numpy(),y_pred[2].numpy(),y_pred[3].numpy(),y_pred[4].numpy(), np.expand_dims(gt,axis=0), y_pred[0].numpy())
