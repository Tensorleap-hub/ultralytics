...
def loss(demo_pred,pred80,pred40,pred20,gt):
...
def gt_visualizer(image: np.ndarray, bb_gt: np.ndarray,data : SamplePreprocessResponse) -> LeapImageWithBBox:
...
def get_matrices(y_pred_0:np.ndarray,pred80: np.ndarray,pred40: np.ndarray,pred20: np.ndarray,preprocess: SamplePreprocessResponse):
    ...
    pred = predictor.postprocess(torch.from_numpy(y_pred_0.copy()))[0]
